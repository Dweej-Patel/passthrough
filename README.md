...
* The phone reports whether its current network routes IPv6. When it doesn't (plenty of home Wi-Fi, some carriers), the helper turns the tunnel's IPv6 routes into reject routes, so apps fall back to IPv4 immediately instead of hanging on IPv6 connections tun2socks accepts but the phone can never make; they flip back when IPv6 returns. The engine runs in its own process (a verified root-only copy of the helper): one that won't stop within 2 seconds is killed, which frees its utun at once, and its stack is logged first so the cause can be found.
